...
# dir              - directory where linux volume is to me mounted
...
# dir2             - second directory where linux volume is to me mounted
...
#
# For ubuntu add this script to etc/rc.local to automate.
#       ruby /home/ubuntu/cloud_init.rb
# For redhat/centos add this script to etc/rc.local to automate.
#       ruby /root/cloud_init.rb
# For windows create a scheduled task at startup.
#       c:/user/administrator/ruby c:/user/administrator/cloud_init.rb
#
#
...
def print_message(message)
   ...
   if RUBY_PLATFORM.index("mswin") != nil or RUBY_PLATFORM.index("i386-mingw32") != nil
      File.open("./cloud.log", 'a') {|f| f.write(message+"\n") }
   else
...
def generate_hosts_file(ec2, server_tag)
   ...
   hosts_filename_bak = "/etc/hosts.bak"
   if RUBY_PLATFORM.index("mswin") != nil or RUBY_PLATFORM.index("i386-mingw32") != nil
      hosts_filename = "C:/Windows/System32/drivers/etc/hosts"
      hosts_filename_bak = "C:/Windows/System32/drivers/etc/hosts.bak" 
   end   
   hosts_file = File.open(hosts_filename)
   File.open(hosts_filename_bak, "w") {|f| f.write hosts_file }
   i = 0
   ...
   if host_file_updated
      File.open(hosts_filename, "w") do |f|
...
def run_chef_client(run_list, solo=false)
   ...
   if RUBY_PLATFORM.index("mswin") == nil and RUBY_PLATFORM.index("i386-mingw32") == nil
      ...
      File.open("C:/chef/cloud_init.json", 'w') {|f| f.write(doc) }
      print_message("#{chef} messages at C:/chef/messages")   
      print_message("#{chef} jsonfile: cloud_init.json")
      system("C:/opscode/chef/bin/#{chef} -c c:/chef/solo.rb -j C:/chef/cloud_init.json -L C:/chef/messages")   
   end
...
def run_chef_client_json(json_string, solo=false)
   ...
   if RUBY_PLATFORM.index("mswin") == nil and RUBY_PLATFORM.index("i386-mingw32") == nil
      ...
      File.open("C:/chef/cloud_init.json", 'w') {|f| f.write(json_string) }
      print_message("#{chef} messages at C:/chef/messages")
      print_message("#{chef} jsonfile: cloud_init.json")    
      system("C:/opscode/chef/bin/#{chef} -c c:/chef/solo.rb -j C:/chef/cloud_init.json -L C:/chef/messages")     
   end
...
print_message("cloud_int.rb - Initialize Cloud Settings")
