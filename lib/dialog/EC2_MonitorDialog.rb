...
class EC2_MonitorDialog  < FXDialogBox
  ...
  def initialize(owner, dimension_value, groupName, report, config, dimension_type="InstanceId")
    ...
    @msg.text = "* Graphs only available when Amazon CloudWatch Monitoring Scripts installed on instance\n Configure Graphs in 'amazon_cloudwatch.json' " if  @dimension_type == "InstanceId"
    @msg.text = "* Graphs only available when Provisioned IOPS Used\n Configure Graphs in 'amazon_cloudwatch.json' " if  @dimension_type == "VolumeId"
  end
    ...
    if measure == "DiskSpaceUtilization"
      begin
        ...
        @response[0]['Dimensions'].each do |m|
          if m['Name'] == "MountPath"
            @dev = m['Value']
            puts "MountPath for DiskSpaceUtilization report is #{@dev}"
            title = "#{title} for #{@dev}"
          end
        end
      rescue
        puts "ERROR: finding MountPath for DiskSpaceUtilization report"
      end
    end
    begin
      if debug
      ...
      if measure == "DiskSpaceUtilization" and @dev != nil and @dev != ""
        options["Dimensions"].push({"Name" => "MountPath","Value" => @dev})
      end
