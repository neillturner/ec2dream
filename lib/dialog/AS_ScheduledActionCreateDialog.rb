...
class AS_ScheduledActionCreateDialog < FXDialogBox

  def initialize(owner, item=nil, group="")
    puts "AS_ScheduledActionCreateDialog.initialize"
    @ec2_main = owner
    @title = ""
    if item == nil 
       @result = ""
    ...
    group_name = FXTextField.new(page1b, 40, nil, 0, :opts => FRAME_SUNKEN)
    group_name.text = @as_name
    group_name_button = FXButton.new(page1b, " ",:opts => BUTTON_TOOLBAR)
