...
class EC2_SecGrp_AuthorizeDialog < FXDialogBox
  ...
  def initialize(owner, current_group, vpc=nil )
    ...
    super(owner, "Security Group Authorization", :opts => DECOR_ALL, :width => 250, :height => 300)
    frame1 = FXMatrix.new(self, 3, :opts => MATRIX_BY_COLUMNS|LAYOUT_FILL)
    @radio1 = FXRadioButton.new(frame1, "Ingress")
    ...
    FXLabel.new(frame1, "" )
    FXLabel.new(frame1, "" )
    @radio2 = FXRadioButton.new(frame1, "Egress")
  ...
  def auth_ip(protocol, from_port, to_port, ip_address, group_auth, group_auth_id)
    @created = false
    begin
      @ec2_main.environment.security_group.create_security_group_rule(@current_group, protocol, from_port, to_port, ip_address, @current_group_id, @current_group, group_auth, group_auth_id, @type)
      @created = true
    rescue
