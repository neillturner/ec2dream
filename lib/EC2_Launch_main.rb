class EC2_Launch
  ...
  def initialize(owner)
    ...
    @launch['IAM_Role_Button'] = FXButton.new(@frame1s, "", :opts => BUTTON_TOOLBAR)
    @launch['IAM_Role_Button'].icon = @magnifier
    @launch['IAM_Role_Button'].tipText = "Select IAM Role"
    @launch['IAM_Role_Button'].connect(SEL_COMMAND) do
      dialog = EC2_RoleDialog.new(@ec2_main)
      dialog.execute
      selected = dialog.selected
      put('IAM_Role',selected) if selected != nil and selected != ""
    end
    FXLabel.new(@frame1, "" )
    ...
    FXLabel.new(@frame1, "" )
    FXLabel.new(@frame1, "User Data (Startup Cmd)")
