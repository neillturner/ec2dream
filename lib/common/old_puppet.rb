           ...
           if answer == MBOX_CLICKED_YES
             if platform != "windows"
              if RUBY_PLATFORM.index("mswin") != nil  or RUBY_PLATFORM.index("i386-mingw32") != nil
				 ...
				 ENV["EC2_USER_PASSWORD"] = password
		     ...
		     ssh_user = "root" if ssh_user == nil or ssh_user == ""
				 ...
				 if private_key != nil and private_key !=""
                    c = "cmd.exe /c \@start \"puppet apply #{ec2_server_name}\" \"#{ENV['EC2DREAM_HOME']}/puppet/puppet_push.bat\"  #{puppet_manifest} #{ec2_server_name} #{ssh_user} #{local_port}"
                     ...
                     c = "cmd.exe /c \@start \"puppet apply #{ec2_server_name}\" \"#{ENV['EC2DREAM_HOME']}/puppet/puppet_push_psw.bat\"  #{puppet_manifest} #{ec2_server_name} #{ssh_user} #{local_port}"
                 end 				 
			puts c
   	         system(c)
   	      else
		     if private_key != nil and private_key !=""
			    ...
			    c = "#{ENV['EC2DREAM_HOME']}/puppet/puppet_push_psw.sh #{puppet_repository} #{puppet_manifest} #{ec2_server_name} #{password} #{ssh_user} #{local_port}"
			 end
			 puts c
   	         system(c)
   	         puts "return message #{$?}"
