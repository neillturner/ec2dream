def remote_desktop(server, password, user="Administrator", port="3389", local_port=nil, bastion_host=nil, bastion_port=nil, bastion_user=nil, bastion_password=nil)
  ...
  wfreerdp_env = ENV['EC2DREAM_WFREERDP']
  if wfreerdp_env != nil and wfreerdp_env.upcase  == 'TRUE'
    ...
    if bastion_host !=nil and bastion_host != ""
      c = c + " /g:#{bastion_host}:#{bastion_port} /gu:#{bastion_user} /gp:\"#{bastion_password}\" /gd:#{bastion_host}" 
    end    
  else
    if local_port !=nil and local_port != ""
